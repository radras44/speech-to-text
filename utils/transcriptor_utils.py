...
def read_batch(audio_paths: List[str],format):
    return [read_audio(audio_path,format=format)
            for audio_path
            in audio_paths]
...
def read_audio(path: str,
               target_sr: int = 16000,format="wav"):

    wav, sr = torchaudio.load(path,format=format)
